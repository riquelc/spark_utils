...
def aplanar_json(df: DataFrame, modo='reduzido') -> DataFrame:
    ...
    while campos_complexos:
        try:
            ...
            logging.info(f"Processando: {nome_coluna}, Tipo: {type(tipo_coluna)}")
            ...
            logging.error(f"Erro processando coluna {nome_coluna}: {str(e)}")
            raise
    
    return df
    """Aplana estruturas JSON aninhadas em um DataFrame Spark, usando os dois últimos níveis dos nomes.
    ...
    Raises:
        ValueError: Se a entrada não for um DataFrame válido
        Exception: Para erros durante o processamento
    """
    if not isinstance(df, DataFrame):
        raise ValueError("A entrada deve ser um DataFrame Spark")
